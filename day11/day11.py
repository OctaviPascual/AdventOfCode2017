...
class HexagonGrid:
    ...
    def run(self):
        current_hexagon = HexagonGrid.INITIAL_HEXAGON

        for direction in self.path:
            current_hexagon = HexagonGrid.DIRECTIONS[direction](current_hexagon)
            current_dist = current_hexagon.distance(HexagonGrid.INITIAL_HEXAGON)
            self.furthest_dist = max(current_dist, self.furthest_dist)

        self.final_dist = current_hexagon.distance(HexagonGrid.INITIAL_HEXAGON)
